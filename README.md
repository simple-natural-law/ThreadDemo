...
[Volatile变量](https://zh.wikipedia.org/wiki/Volatile%E5%8F%98%E9%87%8F)将另一种类型的内存约束应用于变量。编译器通常通过将变量的值加载到寄存器中来优化代码。对于局部变量，这通常不是问题。但是如果变量在另一个线程是可见的，这样的优化可能会阻止其他线程注意到该变量的任何更改。将`volatile`关键字应用于变量会强制编译器在每次使用变量时从内存加载该变量。如果可以随时通过编译器可能无法检测到的外部源更改变量的值，则可以将该变量声明为`volatile`。
