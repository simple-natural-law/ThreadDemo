...
虽然定时器源生成基于时间的通知，但其不是实时的。与输入源一样，定时器源需要与run loop的特定模式相关联。如果定时器源不在当前正在被run loop监听的模式中，其不会触发，直到run loop 以定时器源支持的模式运行。类似地，如果定时器源在run loop处于执行处理例程的过程中触发，则定时器源会等到下一次进入run loop时调用其处理例程。如果run loop根本没有运行，则定时器永远不会触发。
